module aperture-ios-timing-go

go 1.26.3

// tailscale.com is pinned to the apenwarr/aperture branch (commit
// 2a2df99c), which adds the TS_FORCE_CACHE_NETMAP envknob so clients can
// force full netmap caching without the control server granting the
// cache-network-maps node capability. See setupNetmapCacheEnv in tailscale.go.
require tailscale.com v1.99.0-pre.0.20260519223028-2a2df99ca399
