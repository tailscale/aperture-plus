...
/// Value carried by a VM window. The workspace ID keeps the window associated
/// with the browser workspace that created it; the VM UUID keeps every
/// disposable window independent.
...
final class ExperimentalVMController: NSObject, ObservableObject, VZVirtualMachineDelegate {
    ...
    private func makeConfiguration(iso: URL) throws -> VZVirtualMachineConfiguration {
        ...
        // Use Virtualization.framework's standard DHCP/NAT path. The custom
        // TailscaleKit bridge is intentionally disabled until its UDP session
        // lifecycle is repaired; see TODO.vmnet.md.
...
struct ExperimentalVMView: View {
    let id: UUID
    /// Retains the owning workspace so the disposable VM window stays
    /// associated with the workspace that created it.
